...
// `bed` is the FIXED routing surface (smoothed original generated terrain);
// erosion/deposition happen on h but never reroute flow, so incision cannot
// self-reinforce across rounds. Same bed is passed for every round.
function erode(src, bed, size, p, seed, observer) {
...
for (const size of [96, 128]) {
  ...
  for (const s of seeds) {
    ...
    const lo = erode(base, bed, size, P(0.02), s);
    const hi = erode(base, bed, size, P(0.3), s);
    if (variance(hi) > variance(lo)) orderOk++;
    const hi2 = erode(base, bed, size, P(0.3), s);
...
// water monotonicity + termination
{
  ...
  const base = generate(size, s, 6, 0.5, 2, 0.05);
  const bed = smooth(base, size, 2);
  const seen = new Map(); let viol = 0, obs = 0;
  erode(base, bed, size, P(0.3), s, (w, d, step) => {
    obs++;
...
// multi-round: stability + per-round order across many rounds
{
  const size = 96, seeds = [1, 2, 3, 7, 42];
  let stable = true, orderRounds = 0, totalRounds = 0;
  for (const s of seeds) {
    const base = generate(size, s, 6, 0.5, 2, 0.05);
    const bed = smooth(base, size, 2);
    const traj = { lo: [base], hi: [base] };
    let lo = base, hi = base;
    for (let k = 0; k < 10; k++) {
      lo = erode(lo, bed, size, P(0.02), s + k * 99991);
      hi = erode(hi, bed, size, P(0.3), s + k * 99991);
      totalRounds++;
      if (variance(hi) > variance(lo)) orderRounds++;
      if (!Number.isFinite(variance(lo)) || !Number.isFinite(variance(hi)) ||
          variance(lo) > 1 || variance(hi) > 1) stable = false;
    }
  }
  console.log(`multi-round stable=${stable} order ${orderRounds}/${totalRounds}`);
  if (!stable || orderRounds !== totalRounds) allPass = false;
}
console.log(allPass ? 'ALL PASS' : 'FAIL');

// trajectory detail
for (const r of [0.02, 0.3]) {
  const size = 96, s = 1;
  const base = generate(size, s, 6, 0.5, 2, 0.05);
  const bed = smooth(base, size, 2);
  let cur = base; const vs = [variance(cur)];
  for (let k = 0; k < 10; k++) { cur = erode(cur, bed, size, P(r), s + k * 99991); vs.push(variance(cur)); }
  console.log(`r=${r} traj=[${vs.map(v => v.toFixed(4)).join(', ')}]`);
}
