...
function erode(src, size, p, seed, observer) {
  ...
  function brushCarve(px, py, amount) {
    // concentrated single-cell carve, limited by per-cell cumulative cap
    const x = Math.max(0, Math.min(size - 1, Math.round(px)));
    const y = Math.max(0, Math.min(size - 1, Math.round(py)));
    const i = y * size + x;
    amount = Math.min(amount, Math.max(0, cellCap - carved[i]));
    if (amount > 0) { h[i] -= amount; carved[i] += amount; }
    return amount;
  }
  ...
  // Per-cell cumulative-carve depth buffer, relative to the bed at round start.
  // Caps how deeply a point can be incised across all drops of THIS round
  // (reset each round because the bed is re-smoothed), which bounds feedback
  // while still letting higher erosion rates carve more aggressively.
  const carved = new Float64Array(h.length);
  let mn = Infinity, mx = -Infinity;
  for (let i = 0; i < sbed.length; i++) { const v = sbed[i]; if (v < mn) mn = v; if (v > mx) mx = v; }
  const cellCap = (mx - mn) * p.maxCellCarveFrac;
  for (let d = 0; d < p.drops; d++) {
    ...
    for (let step = 0; step < p.maxSteps; step++) {
      ...
      if (sediment > cap || deltaS > 0) {
        ...
        const wanted = Math.max(0, Math.min((cap - sediment) * p.erosion, fall));
        const eroded = brushCarve(px, py, wanted);
        sediment += eroded;
      }
      px = nx; py = ny;
    }
...
function P(r) {
  return { drops: 12000, inertia: 0.05, capK: 2, deposit: 0.1, erosion: r,
    ...
    maxCellCarveFrac: 0.03,
    startSpeed: 1, startWater: 1, maxSteps: 64, minWater: 0.01 };
}
// multi-round stability: re-smooth bed each round from the live terrain
console.log('--- sequential rounds (bed re-smoothed each round), 5 seeds ---');
{
  let allStable = true, orderHolds = true;
  for (const s of [1, 2, 3, 7, 42]) {
    const base = generate(96, s, 6, 0.5, 2, 0.05);
    const series = {};
    for (const r of [0.02, 0.1, 0.3]) {
      let cur = base; const vs = [variance(cur)];
      for (let round = 0; round < 6; round++) {
        cur = erode(cur, 96, P(r), s + round * 99991);
        vs.push(variance(cur));
      }
      series[r] = vs;
    }
    const stable = Object.values(series).every(vs => vs.every(v => Number.isFinite(v) && v < 1));
    // after every round, high erosion variance > low erosion variance
    let order = true;
    for (let k = 1; k <= 6; k++) if (!(series[0.3][k] > series[0.02][k])) order = false;
    console.log(`seed=${s} stable=${stable} orderEveryRound=${order} r0.02=[${series[0.02].map(v=>v.toFixed(4)).join(',')}] r0.3=[${series[0.3].map(v=>v.toFixed(4)).join(',')}]`);
    if (!stable) allStable = false;
    if (!order) orderHolds = false;
  }
  if (!allStable || !orderHolds) process.exitCode = 1;
}
let allPass = true;
