  ...
  for (let d = 0; d < p.drops; d++) {
    ...
    for (let step = 0; step < p.maxSteps; step++) {
      ...
      if (sediment > cap || deltaS > 0) {
        ...
        const eroded = Math.max(0, (cap - sediment) * p.erosion);
...
console.log('=== smoothed routing, narrow carve / wide deposit ===');
for (const erR of [0, 1])
  for (const depR of [2, 3, 4])
    for (const smoothPasses of [1, 2, 3]) {
      const means = rates.map(() => 0); let maxDs = 0, bad = false;
      for (const s of seeds) rates.forEach((r, i) => {
        const hh2 = erode(bases[s], size, P(r, { erodeRadius: erR, depositRadius: depR, smoothPasses }), s);
      ...
      console.log(`erR=${erR} depR=${depR} sm=${smoothPasses} ${bad ? 'UNSTABLE ' : ''}mono=${mono} vars=[${means.map(v => v.toFixed(5)).join(' ')}] dS=${maxDs.toExponential(1)}`);
    }
