// Prototype v7: stable-ish asymmetric cone brushes. Erosion radius 1.5-2.5
// (multi-cell), deposit radius wider; pit-free carve cap = -deltaH.
...
const rates = [0.03, 0.08, 0.15, 0.3];
function P(r, extra) {
  return { drops: DROPS, inertia: 0.05, capK: 4, deposit: 0.1, erosion: r,
    erodeRadius: 1.5, depositRadius: 4, evap: 0.02, gravity: 4,
    startSpeed: 1, startWater: 1, maxSteps: 64, minWater: 0.01, ...extra };
}
for (const erR of [1.5, 2, 2.5])
  for (const depR of [3, 4, 5]) {
    const means = rates.map(() => 0), ranges = rates.map(() => 0);
    let maxDs = 0, bad = false;
    for (const s of seeds) rates.forEach((r, i) => {
      const h = erode(bases[s], size, P(r, { erodeRadius: erR, depositRadius: depR }), s);
      const v = variance(h);
      if (!Number.isFinite(v) || v > 1) bad = true;
      means[i] += v; ranges[i] += range(h) / size;
      maxDs = Math.max(maxDs, Math.abs(sum(h) - sum(bases[s])));
    });
    means.forEach((m, i) => means[i] /= seeds.length);
    ranges.forEach((m, i) => ranges[i] /= seeds.length);
    const mono = means.every((v, i) => !i || v > means[i - 1]);
    console.log(`erR=${erR} depR=${depR} ${bad ? 'UNSTABLE ' : ''}mono=${mono} vars=[${means.map(v => v.toFixed(5)).join(' ')}] rng=[${ranges.map(v => v.toFixed(3)).join(' ')}] dS=${maxDs.toExponential(1)}`);
  }
