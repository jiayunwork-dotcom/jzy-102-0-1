...
// multi-round behavior of the single-round-invariant model (demo usage: click erode repeatedly)
console.log('--- repeated rounds, carveFrac=0.1, each round independent ---');
for (const r of [0.02, 0.1, 0.3]) {
  const rows = [];
  for (const s of [1, 7, 42]) {
    let cur = generate(96, s, 6, 0.5, 2, 0.05); const vs = [variance(cur)];
    for (let k = 0; k < 8; k++) { cur = erode(cur, 96, P(r), s + k * 99991); vs.push(variance(cur)); }
    rows.push(vs);
  }
  const mean = k => rows.reduce((a, v) => a + v[k], 0) / rows.length;
  console.log(`r=${r}: [${rows[0].map((_, k) => mean(k).toFixed(4)).join(', ')}]`);
}
