// Prototype v8: instrument WHERE erosion/deposition happens by elevation quantile,
// then try slope-energy detachment to find a stable variance-increasing variant.
...
function erode(src, size, p, seed, track) {
  ...
  for (let d = 0; d < p.drops; d++) {
    ...
    for (let step = 0; step < p.maxSteps; step++) {
      ...
      // detachment-limited variant: capacity also grows with local slope energy,
      // so drops keep eroding on steep hillslopes (convex noses), not only thalwegs
      const slopeFactor = p.slopePow ? (1 + p.slopeK * Math.pow(Math.min(slopeMag, 2), p.slopePow)) : 1;
      const cap = Math.max(0, -deltaH) * speed * water * p.capK * slopeFactor;
      if (sediment > cap || deltaH > 0) {
        ...
        if (track && amount > 0) track(px, py, amount, slopeMag);
        brushAdd(px, py, amount, depBrush); sediment -= amount;
      } else {
        let amount = Math.max(0, Math.min((cap - sediment) * p.erosion, -deltaH));
        if (track && amount > 0) track(px, py, -amount, slopeMag);
...
console.log('=== slope-energy detachment variants (symmetric brush r=2) ===');
for (const slopeK of [0, 1, 3, 8])
  for (const slopePow of [1, 2]) {
    const means = rates.map(() => 0); let maxDs = 0, bad = false;
    for (const s of seeds) rates.forEach((r, i) => {
      const h = erode(bases[s], size, P(r, { slopeK, slopePow }), s);
      const v = variance(h);
    ...
    console.log(`slopeK=${slopeK} pow=${slopePow} ${bad ? 'UNSTABLE ' : ''}mono=${mono} vars=[${means.map(v => v.toFixed(5)).join(' ')}] dS=${maxDs.toExponential(1)}`);
  }
